...
- [ ] 需要建立自动执行的 CI 检查，覆盖后端测试、前端 lint、前端测试和前端构建
...
- 最新阶段：Phase 11 `Restore Frontend Quality Baseline` 已完成验证，下一步进入 Phase 12 自动化质量门禁建设
...
- 当前仓库前后端测试可运行，WebSocket 访问面和前端质量基线均已收口；后续主要风险集中在 CI 缺位、文档命名对齐和通知链路可靠性
