# Superpowers Documentation Index

## Overview

This directory contains the migrated GSD planning documents converted to superpowers format.

## Specs

### v1.4 Delivery Recovery and Production Hardening (Phases 18-21)

| Phase | Plan | Spec File | Status |
|-------|------|-----------|--------|
| 18 | 01 | `2026-04-10-phase-18-01-delivery-ledger-schema-design.md` | completed |
| 18 | 02 | `2026-04-30-phase-18-02-webhook-ledger-integration-design.md` | completed |
| 18 | 03 | `2026-04-30-phase-18-03-delivery-read-api-design.md` | completed |
| 19 | 01 | `2026-05-01-phase-19-01-delivery-history-ui-design.md` | completed |
| 19 | 02 | `2026-05-01-phase-19-02-recovery-backend-design.md` | completed |
| 19 | 03 | `2026-05-01-phase-19-03-recovery-ui-design.md` | completed |
| 20 | TBD | - | pending |
| 21 | TBD | - | pending |

### v1.3 Notification Reliability and Observability (Phases 14-17)

| Phase | Plan | Spec File | Status |
|-------|------|-----------|--------|
| 14 | 01 | `2026-04-29-phase-14-01-alert-trace-context.md` | completed |
| 14 | 02 | `2026-04-29-phase-14-02-alert-trace-context-lifecycle.md` | completed |
| 15 | 01 | `2026-04-29-phase-15-01-notification-retry-boundaries.md` | completed |
| 15 | 02 | `2026-04-29-phase-15-02-notification-retry-verification.md` | completed |
| 16 | 01 | `2026-04-29-phase-16-01-alert-path-logging-unification.md` | completed |
| 16 | 02 | `2026-04-29-phase-16-02-alert-path-logging-verification.md` | completed |
| 16 | 03 | `2026-04-29-phase-16-03-alert-path-async-panic-context.md` | completed |
| 16 | 04 | `2026-04-29-phase-16-04-alert-path-logging-parse-fix.md` | completed |
| 17 | 01 | `2026-04-29-phase-17-01-truth-docs-verification-scripts.md` | completed |
| 17 | 02 | `2026-04-29-phase-17-02-docs-refresh.md` | completed |
| 17 | 03 | `2026-04-29-phase-17-03-runbook-and-closure.md` | completed |

### v1.2 Alert Pipeline Hardening (Phases 10-13)

| Phase | Plan | Spec File | Status |
|-------|------|-----------|--------|
| 10 | 01 | `2026-04-21-phase-10-01-secure-realtime-alert-access.md` | completed |
| 10 | 02 | `2026-04-21-phase-10-02-secure-realtime-alert-access.md` | completed |
| 11 | 01 | `2026-04-21-phase-11-01-restore-frontend-quality-baseline.md` | completed |
| 11 | 02 | `2026-04-21-phase-11-02-restore-frontend-quality-baseline.md` | completed |
| 12 | 01 | `2026-04-21-phase-12-01-establish-automated-quality-gates.md` | completed |
| 12 | 02 | `2026-04-21-phase-12-02-establish-automated-quality-gates.md` | completed |
| 13 | 01 | `2026-04-21-phase-13-01-harden-notification-delivery-path.md` | completed |
| 13 | 02 | `2026-04-21-phase-13-02-harden-notification-delivery-path.md` | completed |

### v1.1 Enterprise Access Control (Phases 5-9)

| Phase | Plan | Spec File | Status |
|-------|------|-----------|--------|
| 05 | 01 | `2026-04-15-phase-05-01-normalize-role-model.md` | completed |
| 05 | 02 | `2026-04-15-phase-05-02-normalize-role-model.md` | completed |
| 05 | 03 | `2026-04-15-phase-05-03-normalize-role-model.md` | completed |
| 06 | 01 | `2026-04-15-phase-06-01-secure-user-administration.md` | completed |
| 06 | 02 | `2026-04-15-phase-06-02-secure-user-administration.md` | completed |
| 06 | 03 | `2026-04-15-phase-06-03-secure-user-administration.md` | completed |
| 07 | 01 | `2026-04-15-phase-07-01-lock-down-protected-operations.md` | completed |
| 07 | 02 | `2026-04-15-phase-07-02-lock-down-protected-operations.md` | completed |
| 07 | 03 | `2026-04-15-phase-07-03-lock-down-protected-operations.md` | completed |
| 08 | 01 | `2026-04-15-phase-08-01-ship-permission-aware-ui.md` | completed |
| 08 | 02 | `2026-04-15-phase-08-02-ship-permission-aware-ui.md` | completed |
| 08 | 03 | `2026-04-15-phase-08-03-ship-permission-aware-ui.md` | completed |
| 09 | 01 | `2026-04-15-phase-09-01-close-v1-1-milestone-tech-debt.md` | completed |
| 09 | 02 | `2026-04-15-phase-09-02-close-v1-1-milestone-tech-debt.md` | completed |
| 09 | 03 | `2026-04-15-phase-09-03-close-v1-1-milestone-tech-debt.md` | completed |

### v1.0 AI Removal Complete (Phases 1-4)

| Phase | Plan | Spec File | Status |
|-------|------|-----------|--------|
| 01 | 01 | `2026-04-10-phase-01-01-backend-ai-removal-design.md` | completed |
| 01 | 02 | `2026-04-10-phase-01-02-backend-ai-removal-design.md` | completed |
| 01 | 03 | `2026-04-10-phase-01-03-backend-ai-removal-design.md` | completed |
| 02 | TBD | - | pending |
| 03 | TBD | - | pending |
| 04 | TBD | - | pending |

> Archived: `.planning/milestones/`

## Structure

```
docs/superpowers/
├── specs/                    # Converted specs
│   ├── YYYY-MM-DD-phase-XX-YY-design.md
│   └── ...
├── archive/                  # Original GSD docs (after migration)
│   └── .planning/
└── MEMORY.md                 # This index
```

## Migration Progress

- [x] Phase 01 (3 plans) - completed 2026-04-10
- [x] Phase 05-09 (15 plans) - completed 2026-04-15
- [x] Phase 14-17 (11 plans) - completed 2026-04-29
- [x] Phase 18 (3 plans) - completed 2026-05-13
- [x] Phase 19 (3 plans) - completed 2026-05-13
- [ ] Phase 2-13 (archived milestones)
- [ ] Phase 20-21 (pending)